...
p = calculate_tail_frequency(tailed %>% filter(feature == args$feature), untailed %>% filter(feature == args$feature), condition, samps)
write.table(p, paste0(args$outname), col.names = T, row.names = F, quote = F, sep = "\t")
